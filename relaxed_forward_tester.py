#!/usr/bin/env python3
"""
RELAXED 3-MONTH FORWARD TESTING SYSTEM
Train on 1 year, trade for 3 months with RELAXED momentum criteria
Actually make trades instead of holding cash!
"""
...
class RelaxedForwardTester:
    def __init__(self):
        ...
        # RELAXED MOMENTUM PARAMETERS (Will actually trade!)
        self.config = {
            'momentum_6m_threshold': 0.05,     # 5% 6-month momentum (was 15%)
            'momentum_3m_threshold': 0.02,     # 2% 3-month momentum (was 8%)
            'momentum_1m_threshold': 0.01,     # 1% 1-month momentum (was 3%)
            'rsi_max': 85,                     # Less strict on overbought
            'rsi_min': 15,                     # Less strict on oversold
            'volume_multiplier': 1.0,          # Any volume is fine
            'volatility_max': 0.60,            # Allow more volatility
        }
        ...
        self.position_size = 0.18      # 18% per position
    ...
    def calculate_momentum_score(self, data, current_idx):
        """Calculate relaxed momentum score"""
        try:
            current_price = float(data['Close'].iloc[current_idx])
            
            if current_idx < 126:  # Need historical data
                return None
            
            # MOMENTUM CALCULATIONS
            price_6m_ago = float(data['Close'].iloc[current_idx - 126])
            price_3m_ago = float(data['Close'].iloc[current_idx - 63])
            price_1m_ago = float(data['Close'].iloc[current_idx - 21])
    ...
    def should_buy_stock(self, metrics):
        """Relaxed buy criteria - will actually trigger trades"""
        if not metrics:
            return False
        
        # RELAXED momentum criteria - at least 2 of 3 momentum periods positive
        momentum_signals = 0
        if metrics['momentum_6m'] >= self.config['momentum_6m_threshold']:
            momentum_signals += 1
        if metrics['momentum_3m'] >= self.config['momentum_3m_threshold']:
            momentum_signals += 1
        if metrics['momentum_1m'] >= self.config['momentum_1m_threshold']:
            momentum_signals += 1
        
        momentum_good = momentum_signals >= 2  # At least 2 of 3 periods positive
        
        # Relaxed risk criteria
    ...
    def should_sell_stock(self, metrics):
        """Relaxed sell criteria"""
        if not metrics:
            return True
        
        # Sell if momentum really breaks down
        momentum_broken = (
            metrics['momentum_6m'] < -0.05 or   # 6M momentum below -5%
            metrics['momentum_1m'] < -0.03 or   # 1M momentum below -3%
            metrics['rsi'] > 90 or              # Extremely overbought
            metrics['volatility'] > 0.70       # Extremely volatile
        )
        
        return momentum_broken
    
    def run_forward_test(self, start_date=None, end_date=None):
        """Run 3-month forward test with relaxed criteria"""
        ...
        print(f"🚀 RELAXED 3-MONTH FORWARD TEST")
        print("=" * 60)
        print(f"📅 Test Period: {start_date} to {end_date}")
        print(f"📊 Tracking {len(self.stocks)} stocks daily")
        print(f"💰 Starting Capital: ${self.initial_capital:,}")
        print(f"🎯 RELAXED Criteria: More trades, less strict!")
        print("=" * 60)
        
        # Load stock data (only first 10 stocks for speed)
        print("📥 Loading stock data...")
        stock_data = {}
        for symbol in self.stocks[:10]:  # First 10 stocks for speed
            try:
        ...
        for day_idx, current_date in enumerate(trading_days):
            ...
            for symbol in stock_data.keys():
                ...
                if current_idx < 126:  # Need enough history
                    continue
        ...
        print(f"\n🏆 RELAXED 3-MONTH FORWARD TEST RESULTS")
        print("=" * 60)
        print(f"💰 Starting Capital: ${self.initial_capital:,}")
        ...
        for trade in self.trade_log[-5:]:
            ...
            if trade['action'] == 'SELL':
                print(f"{action_color} {trade['date']} {trade['action']} {trade['symbol']} ${trade['price']:.2f} (P&L: {trade['pnl_pct']:+.1f}%)")
            else:
                print(f"{action_color} {trade['date']} {trade['action']} {trade['symbol']} ${trade['price']:.2f}")
...
def main():
    """Run relaxed 3-month forward test"""
    print("🚀 RELAXED INSTITUTIONAL MOMENTUM FORWARD TEST")
    print("=" * 60)
    print("🎯 RELAXED criteria = MORE TRADES!")
    print("📈 5%/2%/1% momentum thresholds (vs 15%/8%/3%)")
    print("💰 Actually invest instead of holding cash")
    print("=" * 60)
    
    tester = RelaxedForwardTester()
    results = tester.run_forward_test()
    
    if results:
        print(f"\n✅ RELAXED FORWARD TEST COMPLETE!")
        print(f"📊 Total Return: {results['total_return']:+.1f}%")
        print(f"💰 Final Value: ${results['final_value']:,.0f}")
        print(f"🎯 Win Rate: {results['win_rate']:.1%}")
        print(f"📈 Made {results['trades']} trades vs 0 with strict criteria!")
